...
data_pre = ddply(data, .(hour, minute, second, event), summarize, total=sum(total))
...
analysis = function(df_ack, df_event,messages, df) {
  mean_ack = mean(df_ack$total)
  sd_ack = mean(df_ack$total)
  mean_event = mean(df_event$total)
  sd_event = mean(df_event$total)
